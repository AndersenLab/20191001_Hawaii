...
<br> **Location review results:** No manual corrections for collection locations.
...
```
**NOTE:** 2 *C. elegans* isolations do not have paired collection data. If we include these isolations there are 8 *C. elegans* isolated not 6.
...
`duplication in genotyping sheet` <- googlesheets::gs_key("1TMgx1TRZ4cgRn24eQCzoy378fKcYRgMrYVlrqgMXVQA") %>%
  ...
  dplyr::select(-c_label, s_label, species_id_ITS2, species_id_rhabditid, lysis_date, pcr_date, ITS2_pcr_product, rhabditid_pcr_product, notes) %>%
  add_count(s_label) %>%
  dplyr::filter(n > 1)
...
**Duplication issue 1:** 9 collections were two separate isolation records exist for the same c_label.<br>
**Solution:** Retained the isolation record based on worm presence. Yes > tracks > no.<br>
If both isolation records indicate "tracks only" or "no" then we retain the earliest record.<br>
In no cases did both isolation records indicate worms were present on c_label.<br>
**Duplication issue 2:** 1 s_label duplicated in the `nematode_isolation_s_labeled_plates.csv`. <br>
**Solution:** This duplication is removed in `step 5` of process fulcrum script. <br>
...
### Remaining concerns as of 20200107
**1.** Two *C. elegans* isolations do not have paired collection data.<br>
**2.** Two living *Caenorhabditis* isolations (S-0632 and S-0669) have not yet been genotyped with the ITS2 primers.<br>
**3.** Two isolations (S-0879 and S-0868) have no species id although they produced ITS2 bands. Need to check them manually again. <br>