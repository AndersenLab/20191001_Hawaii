...
# altitudes <- df1 %>%
#   dplyr::ungroup() %>%
#   dplyr::select(c_label, latitude, longitude) %>%
#   dplyr::rowwise() %>%
#   dplyr::mutate(geonames_altitude = geonames::GNsrtm3(latitude, longitude)$srtm3) %>%
#   dplyr::ungroup()
# 
...
# pull out c_label joined from the genotyping sheet with NAs for collection data. These are likely from errors in genotyping sheet.
