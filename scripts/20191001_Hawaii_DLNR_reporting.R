...
# Export loactions of collections with elegans, briggsae, tropicalis
readr::write_csv(overview_plot_df %>%
                   dplyr::select(collection_id, species_id, substrate, collection_island, collection_location, collection_longitude, collection_latitude) %>%
                   # manuall correction for the cohabitation of c. briggsae and c. tropicalis on C-0422
                   dplyr::mutate(species_id = ifelse(collection_id == "C-0422", "C. tropicalis, C. briggsae", species_id)) %>%
                   dplyr::arrange(species_id, desc(collection_latitude)), "data/DLNR_collection_report.csv")
